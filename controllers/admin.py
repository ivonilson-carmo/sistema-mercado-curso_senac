...
class Administrador():
    ...
    def exibeRelatorioEstoquista(self):
        """ Busca tudo na tabela relatorio_estoquista e exibe cada linha"""
        try:
            print('======= Relatorio Estoquista=======')
    ...
    RELATORIO: {linha[2]} ''')
            ...
            print('\n* Erro 0 - Visualização dos relátorios dos estoquista.')
    
    def exibeRelatorioAuxiliar(self):
        """ Busca tudo na tabela relatorio_auxiliar e exibe cada linha"""
        try:
            print('======= Alteracoes Auxiliar =======')
            
            cursor.execute(' SELECT * FROM alteracoes_auxiliar')
            result = cursor.fetchall()
    ...
    def registrarFornecedor(self):
        """ Obtem e registra informações dos fornecedores """
        try:
            nome = input('Nome do fornecedor: ')
            email = input('Email de contato do fornecedor, caso não possua apenas tecle [enter]: ')
            whatsapp = input('Whatsapp do fornecedor, caso não possua apenas tecle [enter]:  ')
            
            script = 'INSERT INTO fornecedores (nome_fornecedor, email, whatsapp) VALUES (?,?,?)'
            cursor.execute(script, (nome, email, whatsapp) )
            conn.commit()
        
        except KeyboardInterrupt:
            print('\* Saindo do sistema.')
        except Exception:
            print('\n*Erro 0 - Registrando fornecedor')
    ...
    def alteraCadastroMercado(self):
        """ Exibe tudo da tabela cadastro e pergunta o que deseja alterar e realiza a alteração """
        try:
            cursor.execute('''
            SELECT * FROM mercado
            ''')
            ...
            for item in result:
                print(f'0 - ID: {item[0]}')
                print(f'1 - ACESSO ADMINISTRADOR: {item[1]}')
                print(f'2 - CNPJ: {item[2]}')
                print(f'3 - DATA ESTREIA: {item[3]}')
                print(f'4 - DESCRIÇÃO MERCADO: {item[4]}')
                print(f'5 - ENDEREÇO PRINCIPAL: {item[5]}')
                print(f'6 - HORARIO DE FUNCIONAMENTO: {item[6]}')
                print(f'7 - E-mail MERCADO: {item[7]} ')
                print(f'8 - Telefone MERCADO: {item[8]}')

                ops = input('''
                Digite um ou mais items do cadastro que deseja editar(separado por espaços) ou tecle [enter] para deixar como estar
                    >>> ''').strip().split()
                for op in ops:
                    op = int(op)
                    
                    if op == 1:
                        newAcesso = input('Digite seu novo acesso: ')
                        cursor.execute(f'UPDATE mercado SET acesso_administrador = "{newAcesso}" WhERE ID = 1')
                    elif op == 2:
                        newCNPJ = input('Digite o novo CNPJ do mercado: ')
                        cursor.execute(f'UPDATE mercado SET cnpj = "{newCNPJ}" WHERE id = 1')
                    elif op == 3:
                        newDate = input('Digite a data de estreia (padrão: DD-MM-YYYY): ').strip().split('-')
                        newDate = '-'.join(newDate[::-1])
                        cursor.execute(f'UPDATE mercado SET data_hora_estreia = "{newDate}" WhERE ID = 1')
                    
                    elif op == 4:
                        newDesc = input('Digite a nova descrição do mercado: ')
                        cursor.execute(f'UPDATE mercado SET descricao_mercado = "{newDesc}" WhERE ID = 1')
                    
                    elif op == 5:
                        newLoc = input('Digite a nova localização: ')
                        cursor.execute(f'UPDATE mercado SET endereco = "{newLoc}" WhERE ID = 1')
                    
                    elif op == 6:
                        newHorarioFuncionamento = input('Horario de funcionamento: (Padrão: hh:mm - hh:mm): ')
                        cursor.execute(f'UPDATE mercado SET horario_funcionamento = "{newHorarioFuncionamento}" WhERE ID = 1')
                    
                    elif op == 7:
                        newEmail = input('Email: ')
                        cursor.execute(f'UPDATE mercado SET email_mercado = "{newEmail}" WhERE ID = 1')
                    
                    elif op == 8:
                        newNumber = input('Horario de funcionamento: ')
                        cursor.execute(f'UPDATE mercado SET telefone_contato = "{newNumber}" WhERE ID = 1')
            ...
            print('\n* Erro 0 - Alteração dados do registro do mercado.')